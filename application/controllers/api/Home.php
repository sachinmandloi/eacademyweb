...
class Home extends CI_Controller {
    ...
    function login(){
        ...
        if(isset($data['username']) && isset($data['password']) && isset($data['versionCode'])  && isset($data['token'])){
            ...
            $studentDetails = $this->db_model->select_data('*','students use index (id)',array('enrollment_id'=>$enroll_id,'password'=>$pass),1);
            if(!empty($studentDetails)){
                if($studentDetails[0]['status'] == '1'){
                    ...
                    if(!empty($batch_details)){
						...
						if($batch_details[0]['batch_type']==1){
						    $studentData['transactionId']='free';
						}else{
	...
	function student_registration(){
			$data = $_REQUEST;
			if(!empty($data['name']) && !empty($data['email']) && !empty($data['mobile']) && !empty($data['batch_id'])){
				...
				if(!empty($checkBatch)){
					...
					if(empty($prevRecd)){
						...
						if($ins){
        				    ...
        				    if($batch_type[0]['batch_type']==2){
        				        if(!empty($data['amount'])){
        				           ...
        				           //convert Currency
        				           $batchPrice =$data['amount'];
            					   $currency_code_con =$this->general_settings('currency_code');
            					   ...
            					   $amount = $this->convertCurrency($batchPrice, $currency, $currency_code_con);
        				        }
						...
						//$arr = array('status'=>'false', 'msg' =>$this->lang->line('ltr_email_already_msg')); 
						$siteData = array();
					    $siteData['word_for_enroll'] = $this->common->enrollWord;
						$data_arr['admin_id'] = $admin_id;   
						$data_arr['login_status'] = 0;
				...
				if(!empty($batchData)){
					foreach($batchData as $key=>$value){
					   ...
					   if($value['batchType']==2){
    					   ...
    					   $batchData[$key]['batchPriceConvert'] =$this->convertCurrency($batchPrice, $currency_code_con, $currency);
    					   if(!empty($value['batchOfferPrice'])){
    					       $aa =$value['batchOfferPrice'];
    					        $batchData[$key]['batchPriceConvert'] =$this->convertCurrency($aa, $currency_code_con ,$currency);
    					   }
					   }
					}
					
					$arr = array('status'=>'true', 'msg' =>$this->lang->line('ltr_fetch_successfully'),'batchData'=>$batchData);
    ...
    function video_lecture(){
        ...
        if(isset($data['admin_id']) && isset($data['page_no']) && $data['batch_id']){
            ...
            $videos = $this->db_model->select_data('id,admin_id as adminId,title,batch,topic,subject,url,status,added_by as addedBy,added_at as addedAt','video_lectures use index (id)',array('admin_id'=>$admin_id),$limit,array('id','desc'),$like);
            if(!empty($videos)){
    ...
    function db_new_changes(){
        ...
        if(isset($data['admin_id']) && isset($data['student_id']) && isset($data['batch_id'])){
            ...
            if($last_login[0]['last_login_app'] != '0000-00-00 00:00:00'){
                ...
                $videosData = $this->db_model->select_data('id,admin_id as adminId,title,batch,topic,subject,url,status,added_by as addedBy,added_at as addedAt','video_lectures use index (id)',array('admin_id'=>$admin_id,'added_at >= '=>$lastLoginTime),'',array('id','desc'),$likev);
                $videos =array();
